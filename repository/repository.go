...
func (r *GormRepository) UpdateWithMap(uow *UnitOfWork, model interface{}, value map[string]interface{}, queryProcessors ...QueryProcessor) error {
	db, err := applyQueryProcessors(uow.DB, model, queryProcessors...)
	if err != nil {
		return err
	}
	if err := db.Model(model).Updates(value).Error; err != nil {
...
func (r *GormRepository) GetAll(uow *UnitOfWork, out interface{}, queryProcessors ...QueryProcessor) error {
	db, err := applyQueryProcessors(uow.DB, out, queryProcessors...)
	if err != nil {
		return err
	}
...
func (r *GormRepository) Delete(uow *UnitOfWork, model interface{}, queryProcessors ...QueryProcessor) error {
	query := uow.DB.Model(model)

	var err error
	for _, processor := range queryProcessors {
		query, err = processor(query, model)
		if err != nil {
			return apperror.NewValidationError("query", err.Error())
		}
	}
